...
public class SpellCard : Card
{
    ...
    public SpellCard(string name, Sprite image, string info, int mc,
                SpellType spellType = 0, int spellValue = 0,
                SpellTargetType spellTargetType = 0) : base(name, "", info, mc, 0, 0)
    {
        IsSpell = true;
        Spell = spellType;
...
{
    public List<SpellCardData> spellCardDataList;
    public string folderPath = "Assets/Project/Cards";
    ...
    {
        SpellCardData[] spellCardDataArray = Resources.LoadAll<SpellCardData>("Cards");

        if (spellCardDataList == null)
        {
            spellCardDataList = new List<SpellCardData>();
        }

        spellCardDataList.AddRange(spellCardDataArray);
        ...
        foreach (SpellCardData spellCardData in spellCardDataList)
        {
            SpellCard newSpellCard = new SpellCard(
                ...
                spellCardData.image,
                spellCardData.info,
                spellCardData.manaCost,
                spellCardData.spellType,
                spellCardData.spellValue,
                spellCardData.spellTargetType
            );

            CardManager.AllCards.Add(newSpellCard);
        }
    }
